...
public class DeviceListActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		macvector = read.readmac("/interest/config/mac.txt");
		...
		File f = new File(sd, "/interest/config/mac.txt");
		...
		if (f.exists()) {
			if (pairedDevices.size() > 0) {
				findViewById(R.id.title_paired_devices).setVisibility(
						View.VISIBLE);
				for (BluetoothDevice device : pairedDevices) {
					for (int i = 0; i < macvector.size(); i++) {
						System.out.println("equals" + device.getAddress()
								+ "==" + macvector.get(i));
						String address = device.getAddress();
						System.out.println("address is" + address);

						if (address.equalsIgnoreCase((String) macvector.get(i))) {
							isfriend = true;
						}
...
//							+ " - NOT a friend\n" + device.getAddress());
	...
	private OnItemClickListener mDeviceClickListener = new OnItemClickListener() {
		public void onItemClick(AdapterView<?> av, View v, int arg2, long arg3) {
			...
			System.out.println("infoaddress" + address);
